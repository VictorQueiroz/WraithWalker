...
export function normalizeEffectiveSiteConfigs(
  ...
  return normalizeSiteConfigs(
    siteConfigs as Array<Partial<SiteConfig> & { origin: string }>
  );
}

export function haveSameSiteConfigs(
  left: SiteConfig[],
  right: SiteConfig[]
): boolean {
  ...
  return left.every(
    (siteConfig, index) =>
      ...
      siteConfig.dumpAllowlistPatterns.length ===
        right[index]?.dumpAllowlistPatterns.length &&
      siteConfig.dumpAllowlistPatterns.every(
        (pattern, patternIndex) =>
          pattern === right[index]?.dumpAllowlistPatterns[patternIndex]
      )
  );
}
...
export function currentEffectiveSiteConfigs(
  ...
  return normalizeEffectiveSiteConfigs(
    [...state.siteConfigsByOrigin.values()],
    normalizeSiteConfigs
  );
}
...
export function applyEffectiveSiteConfigs(
  ...
  const normalized = normalizeEffectiveSiteConfigs(
    siteConfigs,
    normalizeSiteConfigs
  );
  if (
  ...
  state.siteConfigsByOrigin = new Map(
    normalized.map((siteConfig) => [siteConfig.origin, siteConfig])
  );
  return true;
}
...
export function restoreLocalEffectiveSiteConfigs(
  ...
  return applyEffectiveSiteConfigs(
    state,
    [...state.localSiteConfigsByOrigin.values()],
    normalizeSiteConfigs
  );
}
...
export function toSiteConfigsResult(
  ...
  return {
    ok: true,
    siteConfigs: normalizeEffectiveSiteConfigs(
      siteConfigs,
      normalizeSiteConfigs
    ),
    sentinel
