...
export function createServerBackedRepository(
  ...
  return {
    ...
    writeIfAbsent: (
      payload: Parameters<
        NonNullable<Parameters<typeof createRequestLifecycle>[0]["repository"]>["writeIfAbsent"]
      >[0]
...
export function createLifecycleHarness({ siteConfig, createFixtureDescriptor }: LifecycleHarnessOptions = {}) {
  const state = {
    sessionActive: true,
...
export async function createTempOverridesDir(prefix = "wraithwalker-extension-overrides-"): Promise<string> {
...
export function createFetchPausedParams(
  overrides: { request?: Record<string, unknown> } & Record<string, unknown> = {}
) {
