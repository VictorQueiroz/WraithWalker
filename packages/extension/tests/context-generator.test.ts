...
import { createContextGenerator, inferJsonShape, EDITOR_CONTEXT_FILES } from "../src/lib/context-generator.js";
import { FIXTURE_FILE_NAMES, STATIC_RESOURCE_MANIFEST_FILE } from "../src/lib/constants.js";
import type { SiteConfig, StaticResourceManifest, ResponseMeta } from "../src/lib/types.js";
...
async function writeTextFile(gateway: ReturnType<typeof makeGateway>, root: FileSystemDirectoryHandle, path: string, text: string) {
...
describe("context generator", () => {
  ...
  it("includes static asset summary from RESOURCE_MANIFEST.json", async () => {
    ...
    const generator = createContextGenerator({ rootHandle: asRoot(root), gateway, siteConfigs });
    const markdown = await generator.generate();
  ...
  it("generates .d.ts files from captured JSON API responses", async () => {
    ...
    const meta: ResponseMeta = {
      ...
      mimeType: "application/json", resourceType: "XHR",
    ...
    const typesDir = await root.getDirectoryHandle(".wraithwalker").then((d) => d.getDirectoryHandle("types"));
    const dtsHandle = await typesDir.getFileHandle("https__app.example.com.d.ts");
