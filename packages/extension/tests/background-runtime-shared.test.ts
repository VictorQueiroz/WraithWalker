...
describe("background runtime shared helpers", () => {
  it("recognizes detached-debugger messages case-insensitively", () => {
    expect(isDetachedDebuggerCommandMessage(
  ...
  it("normalizes console timestamps from seconds, milliseconds, and invalid inputs", () => {
    ...
    expect(normalizeConsoleTimestamp(1_775_692_800)).toBe("2026-04-09T00:00:00.000Z");
    expect(normalizeConsoleTimestamp(1_775_692_800_000)).toBe("2026-04-09T00:00:00.000Z");
    expect(normalizeConsoleTimestamp(Number.NaN)).toBe("2026-04-09T12:00:00.000Z");
    expect(normalizeConsoleTimestamp("invalid")).toBe("2026-04-09T12:00:00.000Z");
  });

  it("clips console text and builds browser console entries with sensible defaults", () => {
    const clipped = clipConsoleText("x".repeat(MAX_CONSOLE_ENTRY_TEXT_LENGTH + 2));
  ...
  it("filters background messages and ignores offscreen-only payloads", () => {
    ...
    expect(isBackgroundMessage({ type: "native.open", editorId: "cursor" })).toBe(true);
    expect(isBackgroundMessage({ target: "offscreen", type: "session.start" })).toBe(false);
  ...
  it("extracts explicit error messages and recognizes local-root config errors", () => {
    ...
    expect(isLocalRootConfigUnavailable({ ok: false, error: "No root directory selected." })).toBe(true);
    expect(isLocalRootConfigUnavailable({ ok: false, error: "Root directory access is not granted." })).toBe(true);
    expect(isLocalRootConfigUnavailable({ ok: false, error: "Permission denied." })).toBe(false);
  });
});
