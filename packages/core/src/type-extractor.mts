...
function typeNodeEquals(a: TypeNode, b: TypeNode): boolean {
  ...
  if (a.kind === "object" && b.kind === "object") {
    ...
    return aKeys.every((key, index) => key === bKeys[index] && typeNodeEquals(a.properties[key], b.properties[key]));
  }
  return false;
}
...
export function mergeTypeNodes(a: TypeNode, b: TypeNode): TypeNode {
  ...
  if (a.kind === "object" && b.kind === "object") {
    ...
    for (const key of allKeys) {
      ...
      properties[key] = left && right ? mergeTypeNodes(left, right) : (left || right) as TypeNode;
    }
    return { kind: "object", properties };
  }
...
export function renderTypeNode(node: TypeNode, indent = 0): string {
  ...
  switch (node.kind) {
    ...
    case "union":
      return node.members.map((member) => renderTypeNode(member, indent)).join(" | ");
    case "object": {
      const entries = Object.entries(node.properties);
      if (entries.length === 0) return "{}";
      const innerPad = "  ".repeat(indent + 1);
      const lines = entries.map(([key, value]) => `${innerPad}${key}: ${renderTypeNode(value, indent + 1)};`);
      return `{\n${lines.join("\n")}\n${pad}}`;
    }
  }
}
...
export function renderInterfaceDeclaration(name: string, node: TypeNode): string {
  if (node.kind === "object") {
    ...
    const lines = entries.map(([key, value]) => `  ${key}: ${renderTypeNode(value, 1)};`);
