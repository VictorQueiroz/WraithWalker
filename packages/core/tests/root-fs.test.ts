...
describe("fixture root fs", () => {
  it("resolves paths within the fixture root and blocks escapes", async () => {
    ...
    expect(resolveWithinRoot(root.rootPath, "notes/readme.txt")).toBe(
      root.resolve("notes/readme.txt")
    );
    ...
    expect(rootFs.resolve("notes/readme.txt")).toBe(
      root.resolve("notes/readme.txt")
    );
  ...
  it("reads, writes, and lists files relative to the root", async () => {
    ...
    expect(await rootFs.listDirectories("")).toEqual(
      expect.arrayContaining([".wraithwalker", "data", "notes"])
    );
  ...
  it("writes utf8 and base64 bodies with progress updates", async () => {
    ...
    await rootFs.writeBody("text/readme.txt", {
      body: "hello",
      bodyEncoding: "utf8"
    });

    expect(await rootFs.readText("text/readme.txt")).toBe("hello");
    expect(await rootFs.readBodyAsBase64("binary/font.woff2")).toBe(
      Buffer.from([0, 1, 2, 3, 4, 5]).toString("base64")
    );
    expect(await rootFs.stat("binary/font.woff2")).toEqual(
  ...
  it("copies and removes directory trees within the root", async () => {
    ...
    await rootFs.copyRecursive(
      "fixtures",
      ".wraithwalker/scenarios/baseline/fixtures"
    );
    await expect(rootFs.copyRecursive("missing", "copy")).rejects.toThrow(
      "Path not found: missing"
    );

    expect(
      await rootFs.readText(".wraithwalker/scenarios/baseline/fixtures/app.js")
    ).toBe("console.log('v1');");
    expect(
      await rootFs.readJson<{ ok: boolean }>(
        ".wraithwalker/scenarios/baseline/fixtures/nested/data.json"
      )
    ).toEqual({ ok: true });
