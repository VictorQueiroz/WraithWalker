...
async function tmpdir(prefix = "wraithwalker-core-overrides-"): Promise<string> {
...
describe("syncOverridesDirectory", () => {
  it("syncs Chrome Overrides into .wraithwalker metadata, applies .headers rules, and rebuilds manifests", async () => {
    ...
    const events: Array<{ type: string; requestUrl?: string; reason?: string; bodyPath?: string }> = [];

    await writeFile(dir, ".headers", JSON.stringify([
      {
        applyTo: "*.html",
    ...
    await writeFile(dir, "app.example.com/styles/app.css", "body { color: red; }");
    await writeFile(dir, "app.example.com/scripts/.headers", JSON.stringify([
      {
        applyTo: "*.js",
        headers: [
    ...
    await writeFile(dir, "app.example.com/scripts/app.js", "console.log('hi');");
    await writeFile(dir, "app.example.com/fonts/site.woff2", new Uint8Array([239, 187, 191, 65]));
    await writeFile(dir, "app.example.com/media/clip.mp3", new Uint8Array([73, 68, 51, 4]));
    await writeFile(dir, "app.example.com/images/logo.png", new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]));
    await writeFile(dir, "app.example.com/data/blob.json?lang=en", "{\"ok\":true}");
    ...
    expect(events.some((event) => event.type === "entry-skipped" && event.requestUrl === "notes/readme.txt")).toBe(true);
    expect(events.some((event) => event.type === "entry-start" && event.bodyPath === "app.example.com/index.html")).toBe(true);
    expect(events.some((event) => event.type === "entry-progress" && event.bodyPath === "app.example.com/index.html")).toBe(true);
    expect(events.some((event) => event.type === "entry-complete" && event.bodyPath === "app.example.com/index.html")).toBe(true);
  ...
  it("skips unsupported override paths and invalid .headers files and returns an empty origin set when nothing is usable", async () => {
    ...
    await writeFile(dir, "shape.example.com/.headers", JSON.stringify([{ applyTo: "*.js", headers: [] }]));
    ...
    await writeFile(dir, "site.example.com/longurls/abc123.js", "console.log('hashed');");
    ...
    expect(result.skipped.find((entry) => entry.requestUrl === "broken.example.com/.headers")?.reason).toMatch(
      /^Failed to parse broken\.example\.com\/\.headers: /
    );
    expect(result.skipped.find((entry) => entry.requestUrl === "shape.example.com/.headers")?.reason).toBe(
